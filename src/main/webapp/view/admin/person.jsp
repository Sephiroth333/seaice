...
<body>
    ...
    <table class="tablelist">
        ...
        <c:forEach var="item" items="${datalist }">
	        <tr>
	      	<td><input name="checkxx" type="checkbox" value="${item.getPid() }" /></td>
	      	<td>${item.getPid() }</td>
	        <td>${item.getPname() }</td>
	        <td>${item.getPtitle() }</td>
	        <td><fmt:formatDate value="${item.getUpdatetime()}" type="date" pattern="yyyy-MM-dd HH:mm"/></td>
	        <td><a onclick="javascript:modify(${item.getPid()},false)" href="#" class="modify"> 修改</a>
		    	<a onclick="javascript:del(${item.getPid()})" href="#" class="tablelink removeBtn"> 删除</a>
		    </td>
	        </tr>
        </c:forEach>       	
        </tbody>
    	...
    	<div class="tipinfo" style="height: 309px; overflow-y: scroll;">
		  	<div class="tipright">
		  		...
		  		<form id="add_form">
			  		<div class="group">
		                <label>姓名：</label>
		                <input name="pname" type="text" class="dfinput" />
	            	</div>
		            <div class="group">
		                <label>标签：</label>
		                <input name="ptitle" type="text" class="dfinput" />
		            </div>
		            <div class="group editorWrap">
		                <label>简介：</label>			
						<!-- 加载编辑器的容器 --> 
						<script id="pcontent" name="pcontent" type="text/plain" style="margin-left:85px;"></script>
		            </div>		  		
		    	</form>
		    	<div class="tipbtn">
    	...
    	<div class="tipinfo" style="height: 309px; overflow-y: scroll;">
		  	<div class="tipright">
		  		...
		  		<form id="modify_form">
			  		...
			  		<div class="group">
		                <label>姓名：</label>
		                <input name="pname2" type="text" class="dfinput" />
	            	</div>
		            <div class="group">
		                <label>标签：</label>
		                <input name="ptitle2" type="text" class="dfinput" />
		            </div>
		            <div class="group editorWrap">
		                <label>简介：</label>			
						<!-- 加载编辑器的容器 --> 
						<script id="pcontent2" name="pcontent2" type="text/plain" style="margin-left:85px;"></script>
		            </div>		  		
		    	</form>
		    	<div class="tipbtn">
    ...
    var ue = UE.getEditor('pcontent',{
    	initialFrameWidth:600,
    	initialFrameHeight:200,
    	scaleEnabled:true,
    	enterTag:''
    });
    var ue2 = UE.getEditor('pcontent2',{
    	initialFrameWidth:600,
    	initialFrameHeight:200,
    	scaleEnabled:true,
    	enterTag:''
    ...
    function pagejump(nowpage){
    	window.location.href="personManage?pageNow="+nowpage;
    }
    function pagejump2(){
    ...
    function modify(sampleid){
    	$.ajax({
    		type:"post",
  			url:"personView?id="+sampleid,
  			dataType:"json",
  			success: function(objs){
  				if(objs.success){
  					var obj=objs.data;
  					$("input[name='modifydtID']").val(obj.pid);
  					$("input[name='pname2']").val(obj.pname);
  					$("input[name='ptitle2']").val(obj.ptitle);
  					ue2.setContent(obj.pcontent);
